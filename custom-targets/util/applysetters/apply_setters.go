...
func (as *ApplySetters) Apply(path string) error {
	...
	yamlFile, err := nodes[0].String()
	if err != nil {
		return err
	}

	if err := os.WriteFile(path, []byte(yamlFile), os.ModePerm); err != nil {
		return err
	}
	return nil
}
...
func Decode(params map[string]string, fcd *ApplySetters) {
